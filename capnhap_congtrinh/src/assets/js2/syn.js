    ...
    $.getJSON( url, function( data ) { 
        localDB.transaction(function (transaction) {
            ...
            for(var i = 0; i < len; i++) {
                ...
                transaction.executeSql('INSERT INTO solieu_congtrinh (id,trangthai,baocaohuhong,idhuyen,idxa,lon,lat,idusers,ngaythunhan) VALUES (?,?,?,?,?,?,?,?,?)',[id,trangthai,baocaohuhong,idhuyen,idxa,lon,lat,idusers,ngaythunhan]);
            }
        });
    });
}
function load_solieu_thuyvan(){
	var url=urlsyn+'?tbl=solieu_thuyvan';
    $.getJSON( url, function( data ) { 
        localDB.transaction(function (transaction) {
            var len = data.length;
			transaction.executeSql('DELETE FROM solieu_thuyvan',[]);
            for(var i = 0; i < len; i++) {
                var id=data[i].id;
				var doman=data[i].doman;
                var mucnuoc=data[i].mucnuoc;
				var idhuyen=data[i].idhuyen;
				var idxa=data[i].idxa;
				var lon=data[i].lon;
				var lat=data[i].lat;
				...
				var ngaythunhan=data[i].ngaythunhan;
                //console.log('we insert ' + ten);
                transaction.executeSql('INSERT INTO solieu_thuyvan (id,doman,mucnuoc,idhuyen,idxa,lon,lat,idusers,ngaythunhan) VALUES (?,?,?,?,?,?,?,?,?)',[id,doman,mucnuoc,idhuyen,idxa,lon,lat,idusers,ngaythunhan]);
            }
        });
    });
}
